...
**解法1**思路：从前向后扫描数组元素，标记一个可能的结果，然后扫描数组进行确认。
...
int  main(){
    int test1[]={0,5,5,3,5,7,5,5},length1=8;
    int test2[]={0,5,5,3,5,7},length2=6;
    int test3[]={1,2,3,4,5,6,7},length3=7;
    int ret=0;
    ret=findPivot(test1,length1);
    printf("%d\n",ret);

    ret=findPivot(test2,length2);
    printf("%d\n",ret);

    ret=findPivot(test3,length3);
    printf("%d\n",ret);
}
```

**解法2**：思路：将数组进行排序如果存在某个数字出现的次数超过一半，排序后length/2的位置就是该元素，扫描数字对答案进行确认。
...
#include<stdio.h>
#include<stdlib.h>
int findMinPositive(int array[],int length){
    bool *dp=(bool*)malloc(sizeof(bool)*length);
    for(int i=0;i<length;i++){
        dp[i]=false;
    }
    for(int i=0;i<length;i++){
...
int  main(){
    ...
    int test3[]={1,2,3,4,5,6,7},length3=7;
    int ret=0;
    ret=findMinPositive(test1,length1);
    printf("%d\n",ret);

    ret=findMinPositive(test2,length2);
    printf("%d\n",ret);

    ret=findMinPositive(test3,length3);
    printf("%d\n",ret);
}
```
...
``` c
int firstMissingPositive(int* array, int length){
    // 解法2 
    for(int i=0;i<length;i++){
